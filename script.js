...
// State for managing currently playing audio
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Attach event listeners for date/week changes
...
function updateInstructionsOnDateChange() {
    ...
    if (isManualMode) {
        ...
        // Find all visible option cards on the page
        const optionCards = document.querySelectorAll('#optionsContainer .option-card');
        optionCards.forEach(card => {
            // Get the index of the option from the card's ID (e.g., 'option-card-0' -> 0)
            const optionIndex = parseInt(card.id.replace('option-card-', ''));
            
            // Check if the data for this option exists
            if (allCalculatedOptions[optionIndex]) {
                const summaryContainer = card.querySelector('.border-t.pt-4');
                if (summaryContainer) {
                    // Re-render the instruction part of the card with the updated date/week value
